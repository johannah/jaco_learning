...
def convert_tool_pose(current_tool_pose, unit, relative, position,
    ...
    if unit == 'mq':
        if relative:
            ...
            orientation_xyz_list = [
                orientation_XYZ[i] + current_orientation_q[i]
                for i in range(0, 3)
            ]
        ...
        if relative:
            orientation_deg_list = list(
                map(math.degrees, current_orientation_q))
            orientation_deg = [
                orientation_[i] + orientation_deg_list[i] for i in range(0, 3)
            ]
        else:
        ...
        if relative:
            ...
            orientation_rad = [
                orientation_[i] + orientation_rad_list[i] for i in range(0, 3)
            ]
        else:
...
def convert_tool_pose_old_version(current_tool_pose, unit, relative, position,
                                  orientation):
    """
